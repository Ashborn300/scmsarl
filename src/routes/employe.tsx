...
type BlocOrganigramme = { id: string; titre: string; niveau: number; couleur: "bleu" | "vert" | "orange" | "violet" | "turquoise"; parentId?: string };
...
function OrgNodeEmploye({ bloc, blocs }: { bloc: BlocOrganigramme; blocs: BlocOrganigramme[] }) { const enfants = blocs.filter((item) => item.parentId === bloc.id).slice(0, 3); return <div className="org-branch"><div className={`org-node org-${bloc.couleur}`}><span className="org-icon"><UsersRound className="size-6" /></span><strong>{bloc.titre}</strong></div>{enfants.length > 0 && <div className="org-children">{enfants.map((enfant) => <OrgNodeEmploye key={enfant.id} bloc={enfant} blocs={blocs} />)}</div>}</div>; }
