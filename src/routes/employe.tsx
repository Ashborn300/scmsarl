...
function Dashboard({ role, stats, employe, chantiers, presences, setOnglet }: any) { return <div className="space-y-6"><section className="dashboard-hero overflow-hidden rounded-3xl p-6 shadow-tool"><p className="text-sm font-black uppercase opacity-85">{role === "admin" ? "Vue direction" : "Espace personnel"}</p><h3 className="mt-2 text-3xl font-black sm:text-4xl">SCM SARL — Tableau de bord</h3><div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-card/18 p-4"><p className="text-2xl font-black">{stats.totalChantiers}</p><p className="text-xs font-bold opacity-85">Chantiers suivis</p></div><div className="rounded-2xl bg-card/18 p-4"><p className="text-2xl font-black">{stats.chantiersActifs}</p><p className="text-xs font-bold opacity-85">Actifs</p></div><div className="rounded-2xl bg-card/18 p-4"><p className="text-2xl font-black">{stats.presences}</p><p className="text-xs font-bold opacity-85">Présences</p></div></div></section><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><CarteStat tone="tool-blue" titre="Projets" valeur={stats.totalProjets} icone={BriefcaseBusiness} /><CarteStat tone="tool-purple" titre="Employés" valeur={stats.totalEmployes} icone={UsersRound} /><CarteStat tone="tool-orange" titre="Chantiers" valeur={stats.totalChantiers} icone={HardHat} /><CarteStat tone="tool-green" titre="Chantiers actifs" valeur={stats.chantiersActifs} icone={CheckCircle2} /><CarteStat tone="tool-coral" titre="Présences" valeur={stats.presences} icone={ClipboardCheck} /></div>{role === "admin" && <AdminPresenceCharts presences={presences} chantiers={chantiers} />}{employe && <div className="grid gap-4 lg:grid-cols-3"><CarteStat tone="tool-teal" titre="Salaire total" valeur={devise(employe.salaire_total ?? employe.salaire)} icone={ClipboardList} /><CarteStat tone="tool-green" titre="Salaire reçu" valeur={devise(employe.salaire_recu || 0)} icone={CheckCircle2} /><CarteStat tone="tool-orange" titre="Salaire restant" valeur={devise(employe.salaire_restant ?? 0)} icone={CalendarDays} /></div>}<div className="grid gap-5 xl:grid-cols-2"><Apercu titre={role === "admin" ? "Chantiers récents" : "Mes chantiers"} action="Voir les chantiers" onClick={() => setOnglet("chantiers")} items={chantiers.slice(0, 4).map((c: Chantier) => ({ titre: c.nom_chantier, meta: `${c.localisation || "Localisation non définie"} • ${c.statut}` }))} /><Apercu titre="Présences récentes" action="Voir les présences" onClick={() => setOnglet("presences")} items={presences.slice(0, 4).map((p: Presence) => ({ titre: dateFr(p.date), meta: p.notes || "Rapport quotidien" }))} /></div></div>; }
