...
function EmployePage() {
  ...
  if (!session) return (<><InstallAppDialog /><LoginScreen identifiant={identifiant} setIdentifiant={setIdentifiant} connecter={connecter} saving={sauvegarde} message={message} chargement={false} /></>);
    ...
    <main className="dashboard-shell min-h-screen text-foreground">
      <div className="flex min-h-screen">
        ...
        <section className="flex-1 px-4 pb-24 pt-5 sm:px-6 lg:px-8 lg:pb-8">
          <header className="dashboard-card mb-6 flex flex-col gap-4 rounded-3xl p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><button className="tool-action lg:hidden" onClick={() => setMenuOuvert(true)} aria-label="Ouvrir"><Menu className="size-5" /></button><div className="inline-flex size-12 items-center justify-center overflow-hidden rounded-2xl bg-card shadow-tool ring-1 ring-border"><img src={scmCompanyLogo} alt="Logo SCM SARL" className="h-full w-full object-contain" /></div><div><p className="text-xs font-black uppercase tracking-wide text-muted-foreground">Espace entreprise</p><h2 className="text-2xl font-black sm:text-3xl">{titreOnglet}</h2></div></div><div className="flex items-center gap-2">{isAdmin && ["projets", "employes", "chantiers", "annonces"].includes(onglet) && <button className="primary-action" onClick={() => ouvrirCreation(onglet as any)}><Plus className="size-4" /> Nouveau</button>}<button type="button" className="tool-action danger" onClick={deconnecter} aria-label="Déconnexion" title="Déconnexion"><LogOut className="size-4" /></button></div></header>
          ...
          {chargement ? <div className="flex min-h-[50vh] items-center justify-center rounded-3xl border border-border bg-card"><Loader2 className="size-8 animate-spin text-primary" /></div> : onglet === "dashboard" ? <Dashboard role={session.role} session={session} stats={stats} employe={employeConnecte} chantiers={chantiersVisibles} presences={presencesVisibles} annonces={annoncesVisibles} jours={joursVisibles} setOnglet={setOnglet} voirAnnonce={(id: string) => setDetail({ type: "annonces", id })} masquerAnnonce={masquerAnnonce} admin={isAdmin} /> : <div className="space-y-5">
      ...
      {detail && <Modal titre="Détails" fermer={() => setDetail(null)}><Details detail={detail} projets={projetsVisibles} employes={employesVisibles} chantiers={chantiersVisibles} presences={presencesVisibles} annonces={annoncesVisibles} admin={isAdmin} role={session.role} viewerId={session.employeId} saving={sauvegarde} televerserPhotoProfil={televerserMaPhotoProfil} retirerPhotoProfil={retirerMaPhotoProfil} televerserImagesChantier={televerserImagesChantierDirect} retirerImageChantier={retirerImageChantierDirect} modifier={() => detail.type !== "presences" && detail.type !== "annonces" && ouvrirEdition(detail.type, detail.id)} supprimer={() => detail.type === "annonces" ? supprimerAnnonce(detail.id) : detail.type !== "presences" && supprimer(detail.type, detail.id)} /></Modal>}
...
function OrganigrammeEmployes({ organigramme }: { organigramme?: OrganigrammeEntreprise }) {
  const blocs = (organigramme?.blocs || []).filter((b) => b.id !== "image-organigramme");
  const imageUrlLegacy = String(organigramme?.blocs?.[0]?.image_url || "");
  const hasTree = blocs.length > 0;
  const showLegacyImage = !hasTree && imageUrlLegacy && organigramme?.blocs?.[0]?.id === "image-organigramme";
  return <section className="dashboard-card rounded-3xl p-4 sm:p-6"><div className="mb-5 flex items-center gap-3"><span className="tool-organization-chart flex size-12 items-center justify-center rounded-2xl bg-tool-gradient text-tool-foreground shadow-tool"><Network className="size-6" /></span><div><p className="text-xs font-black uppercase tracking-wide text-muted-foreground">SCM SARL</p><h3 className="text-2xl font-black">{organigramme?.titre || "Organigramme de l’entreprise"}</h3></div></div>{hasTree ? <><div className="org-chart"><img src={scmCompanyLogo} alt="Logo SCM SARL" className="org-logo" /><h2>ORGANIGRAMME</h2><div className="org-line" /><ArbreOrganigramme blocs={blocs} /></div><p className="mt-4 text-center text-sm font-black text-muted-foreground">{organigramme?.description}</p></> : showLegacyImage ? <><div className="org-chart org-image-chart"><img src={scmCompanyLogo} alt="Logo SCM SARL" className="org-logo" /><h2>ORGANIGRAMME</h2><div className="org-line" /><img src={imageUrlLegacy} alt="Organigramme SCM SARL" className="org-uploaded-image" loading="lazy" /></div><p className="mt-4 text-center text-sm font-black text-muted-foreground">{organigramme?.description}</p></> : <p className="rounded-2xl bg-muted p-5 text-sm font-bold text-muted-foreground">Aucun organigramme publié pour le moment.</p>}</section>;
}
function ArbreOrganigramme({ blocs }: { blocs: BlocOrganigramme[] }) {
  const racines = blocs.filter((b) => !b.parentId || !blocs.some((p) => p.id === b.parentId));
  return <ul className="org-tree">{racines.map((r) => <NoeudOrganigramme key={r.id} bloc={r} blocs={blocs} />)}</ul>;
}
function NoeudOrganigramme({ bloc, blocs }: { bloc: BlocOrganigramme; blocs: BlocOrganigramme[] }) {
