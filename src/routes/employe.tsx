...
function OrgNodeEmploye({ bloc, blocs }: { bloc: BlocOrganigramme; blocs: BlocOrganigramme[] }) { const enfants = blocs.filter((item) => item.parentId === bloc.id).slice(0, 3); const enfantsBas = enfants.filter((item) => item.position !== "cote"); const enfantsCote = enfants.filter((item) => item.position === "cote"); return <div className="org-branch"><div className="org-node-row"><div className={`org-node org-${bloc.couleur}`}><span className="org-icon"><UsersRound className="size-6" /></span><strong>{bloc.titre}</strong></div>{enfantsCote.length > 0 && <div className="org-side-children">{enfantsCote.map((enfant) => <OrgNodeEmploye key={enfant.id} bloc={enfant} blocs={blocs} />)}</div>}</div>{enfantsBas.length > 0 && <div className="org-children">{enfantsBas.map((enfant) => <OrgNodeEmploye key={enfant.id} bloc={enfant} blocs={blocs} />)}</div>}</div>; }
