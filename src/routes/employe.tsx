...
const employeInitial: EmployeForm = { nom_complet: "", poste: "", matricule: "", telephone: "", adresse: "", salaire_total: "", salaire_recu: "0", role: "employe", statut: "actif", chantier_assigne: "", peut_voir_budget: false };
...
function EmployePage() {
  ...
  async function connecter(event: React.FormEvent) {
    ...
    setSauvegarde(true);
    if (loginMode === "admin") {
      const passHash = await sha256(motDePasse);
      const { data } = await db.from("admin_accounts").select("*").eq("username", identifiant.trim()).eq("password_hash", passHash).eq("actif", true).maybeSingle();
  ...
  const ouvrirEdition = (type: Exclude<ModeEdition, null>["type"], id: string) => {
    ...
    if (type === "employes") { const item = employes.find((e) => e.id === id); if (item) setFormEmploye({ ...item, salaire_total: String(item.salaire_total ?? item.salaire ?? ""), salaire_recu: String(item.salaire_recu || 0), chantier_assigne: item.chantier_assigne || "", role: item.role || "employe", peut_voir_budget: !!item.peut_voir_budget }); }
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="flex min-h-screen">
        ...
        <section className="flex-1 px-4 pb-24 pt-5 sm:px-6 lg:px-8 lg:pb-8">
          ...
          {chargement ? <div className="flex min-h-[50vh] items-center justify-center rounded-3xl border border-border bg-card"><Loader2 className="size-8 animate-spin text-primary" /></div> : onglet === "dashboard" ? <Dashboard role={session.role} stats={stats} employe={employeConnecte} chantiers={chantiersVisibles} presences={presencesVisibles} setOnglet={setOnglet} /> : <div className="space-y-5">
            ...
            {onglet === "projets" && <ListeProjets projets={projetsFiltres} admin={isAdmin} voir={(id) => setDetail({ type: "projets", id })} modifier={(id) => ouvrirEdition("projets", id)} supprimer={(id) => supprimer("projets", id)} />}
            {onglet === "employes" && <ListeEmployes employes={employesFiltres} chantiers={chantiers} admin={isAdmin} showSalary={isAdmin || !isChef} voir={(id) => setDetail({ type: "employes", id })} modifier={(id) => ouvrirEdition("employes", id)} supprimer={(id) => supprimer("employes", id)} />}
            {onglet === "chantiers" && <ListeChantiers chantiers={chantiersFiltres} projets={projets} employes={employes} admin={isAdmin} viewerRole={session.role} viewerId={session.employeId} voir={(id) => setDetail({ type: "chantiers", id })} modifier={(id) => ouvrirEdition("chantiers", id)} supprimer={(id) => supprimer("chantiers", id)} />}
            {onglet === "presences" && <PresencesSection admin={isAdmin} chef={isChef} presences={presencesFiltrees} chantiers={chantiers} employes={employes} chefs={chefOptions} filtreDate={filtreDate} setFiltreDate={setFiltreDate} filtreChantier={filtreChantier} setFiltreChantier={setFiltreChantier} filtreEmploye={filtreEmploye} setFiltreEmploye={setFiltreEmploye} filtreChef={filtreChef} setFiltreChef={setFiltreChef} voir={(id) => setDetail({ type: "presences", id })} presenceDate={presenceDate} setPresenceDate={setPresenceDate} presenceChantier={presenceChantier || chantierPresence?.id || ""} setPresenceChantier={setPresenceChantier} presenceNotes={presenceNotes} setPresenceNotes={setPresenceNotes} employesPresence={employesPresence} presenceStatuts={presenceStatuts} setPresenceStatuts={setPresenceStatuts} submit={enregistrerPresence} saving={sauvegarde} chantiersVisibles={chantiersVisibles} />}
          </div>}
        </section>
      </div>
      ...
      {edition && <Modal titre={edition.id ? "Modifier" : "Créer"} fermer={() => setEdition(null)}>{edition.type === "projets" && <FormProjet form={formProjet} setForm={setFormProjet} onSubmit={enregistrerProjet} saving={sauvegarde} />}{edition.type === "employes" && <FormEmploye form={formEmploye} setForm={setFormEmploye} chantiers={chantiers} onSubmit={enregistrerEmploye} saving={sauvegarde} />}{edition.type === "chantiers" && <FormChantier form={formChantier} setForm={setFormChantier} projets={projets} employes={employes} onSubmit={enregistrerChantier} saving={sauvegarde} televerserImages={televerserImages} retirerImage={retirerImage} />}</Modal>}
...
function LoginScreen({ mode, setMode, identifiant, setIdentifiant, motDePasse, setMotDePasse, connecter, saving, message, chargement }: any) {
  return <main className="construction-grid flex min-h-screen items-center justify-center bg-background p-4"><section className="w-full max-w-xl rounded-3xl border border-border bg-card p-6 shadow-document"><div className="tool-blue mb-5 inline-flex size-14 items-center justify-center rounded-2xl bg-tool-gradient text-tool-foreground shadow-tool"><ShieldCheck className="size-7" /></div><h1 className="text-3xl font-black">SCM SARL</h1><p className="mt-2 text-sm text-muted-foreground">Connexion sécurisée à l’espace entreprise.</p><div className="mt-6 grid grid-cols-2 gap-2 rounded-2xl bg-muted p-1"><button className={`rounded-xl px-3 py-2 text-sm font-black ${mode === "matricule" ? "bg-card shadow-document" : "text-muted-foreground"}`} onClick={() => setMode("matricule")}>Matricule</button><button className={`rounded-xl px-3 py-2 text-sm font-black ${mode === "admin" ? "bg-card shadow-document" : "text-muted-foreground"}`} onClick={() => setMode("admin")}>Admin</button></div><form className="mt-6 space-y-4" onSubmit={connecter}><Champ label={mode === "admin" ? "Identifiant admin" : "Matricule employé"}><input className="form-control" value={identifiant} onChange={(e) => setIdentifiant(e.target.value)} placeholder={mode === "admin" ? "admin" : "Ex: SCM-001"} /></Champ>{mode === "admin" && <Champ label="Mot de passe"><input type="password" className="form-control" value={motDePasse} onChange={(e) => setMotDePasse(e.target.value)} placeholder="Mot de passe administrateur" /></Champ>}{message && <p className="rounded-xl bg-muted p-3 text-sm font-bold text-foreground">{message}</p>}<button className="primary-action w-full" disabled={saving || chargement}>{saving || chargement ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />} Se connecter</button></form><p className="mt-4 text-xs text-muted-foreground">Compte initial : admin / SCM2026!. Changez-le dès que possible.</p></section></main>;
}
...
function ListeEmployes({ employes, chantiers, admin, showSalary, voir, modifier, supprimer }: any) { return <div className="grid gap-4 xl:grid-cols-2">{employes.map((e: Employe) => <article key={e.id} className="rounded-2xl border border-border bg-card p-5 shadow-document"><div className="flex justify-between gap-4"><div><span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-black text-primary">{e.role === "chef_chantier" ? "Chef de chantier" : "Employé"}</span><h3 className="mt-3 text-xl font-black">{e.nom_complet}</h3><p className="mt-1 text-sm text-muted-foreground">{e.poste} • {e.matricule}</p></div><Actions admin={admin} voir={() => voir(e.id)} modifier={() => modifier(e.id)} supprimer={() => supprimer(e.id)} /></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><Info icone={UserRound} label="Téléphone" valeur={e.telephone || "Non défini"} />{showSalary && <Info icone={ClipboardList} label="Salaire restant" valeur={devise(e.salaire_restant ?? 0)} />}<Info icone={HardHat} label="Chantier" valeur={nomChantier(chantiers, e.chantier_assigne)} /><Info icone={MapPin} label="Adresse" valeur={e.adresse || "Non définie"} /></div></article>)}</div>; }
