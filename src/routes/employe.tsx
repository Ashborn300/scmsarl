...
type BlocOrganigramme = { id: string; titre: string; niveau: number; couleur: "bleu" | "vert" | "orange" | "violet" | "turquoise"; parentId?: string; position?: "bas" | "cote" };
...
function normaliserBlocsOrganigramme(blocs: BlocOrganigramme[]) { return blocs.map((bloc) => { if (bloc.niveau === 0 || bloc.parentId) return bloc; const parent = blocs.find((item) => item.niveau === bloc.niveau - 1); return parent ? { ...bloc, parentId: parent.id } : bloc; }); }
function OrgNodeEmploye({ bloc, blocs }: { bloc: BlocOrganigramme; blocs: BlocOrganigramme[] }) { const enfants = blocs.filter((item) => item.parentId === bloc.id).slice(0, 3); const enfantsBas = enfants.filter((item) => item.position !== "cote"); const enfantsCote = enfants.filter((item) => item.position === "cote"); const enfantsGauche = enfantsCote.filter((_, index) => index % 2 === 0); const enfantsDroite = enfantsCote.filter((_, index) => index % 2 === 1); return <div className="org-branch"><div className="org-node-row">{enfantsGauche.length > 0 && <div className="org-side-children org-side-left">{enfantsGauche.map((enfant) => <OrgNodeEmploye key={enfant.id} bloc={enfant} blocs={blocs} />)}</div>}<div className={`org-node org-${bloc.couleur}`}><span className="org-icon"><UsersRound className="size-6" /></span><strong>{bloc.titre}</strong></div>{enfantsDroite.length > 0 && <div className="org-side-children org-side-right">{enfantsDroite.map((enfant) => <OrgNodeEmploye key={enfant.id} bloc={enfant} blocs={blocs} />)}</div>}</div>{enfantsBas.length > 0 && <div className="org-children">{enfantsBas.map((enfant) => <OrgNodeEmploye key={enfant.id} bloc={enfant} blocs={blocs} />)}</div>}</div>; }
function OrganigrammeEmployes({ organigramme }: { organigramme?: OrganigrammeEntreprise }) { const blocs = normaliserBlocsOrganigramme(organigramme?.blocs || []); const racines = blocs.filter((bloc) => bloc.niveau === 0 || !bloc.parentId).slice(0, 3); return <section className="dashboard-card rounded-3xl p-4 sm:p-6"><div className="mb-5 flex items-center gap-3"><span className="tool-organization-chart flex size-12 items-center justify-center rounded-2xl bg-tool-gradient text-tool-foreground shadow-tool"><Network className="size-6" /></span><div><p className="text-xs font-black uppercase tracking-wide text-muted-foreground">SCM SARL</p><h3 className="text-2xl font-black">{organigramme?.titre || "Organigramme de l’entreprise"}</h3></div></div>{blocs.length ? <><div className="org-chart"><img src={scmCompanyLogo} alt="Logo SCM SARL" className="org-logo" /><h2>ORGANIGRAMME</h2><div className="org-line" /><div className="org-scroll"><div className="org-roots">{racines.map((bloc) => <OrgNodeEmploye key={bloc.id} bloc={bloc} blocs={blocs} />)}</div></div></div><p className="mt-4 text-center text-sm font-black text-muted-foreground">{organigramme?.description}</p></> : <p className="rounded-2xl bg-muted p-5 text-sm font-bold text-muted-foreground">Aucun organigramme publié pour le moment.</p>}</section>; }
