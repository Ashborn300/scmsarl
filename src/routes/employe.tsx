...
function OrganigrammeEmployes({ organigramme }: { organigramme?: OrganigrammeEntreprise }) { const blocs = normaliserBlocsOrganigramme(organigramme?.blocs || []); const racines = blocs.filter((bloc) => bloc.niveau === 0 || !bloc.parentId).slice(0, 3); return <section className="dashboard-card rounded-3xl p-4 sm:p-6"><div className="mb-5 flex items-center gap-3"><span className="tool-organization-chart flex size-12 items-center justify-center rounded-2xl bg-tool-gradient text-tool-foreground shadow-tool"><Network className="size-6" /></span><div><p className="text-xs font-black uppercase tracking-wide text-muted-foreground">SCM SARL</p><h3 className="text-2xl font-black">{organigramme?.titre || "Organigramme de l’entreprise"}</h3></div></div>{blocs.length ? <><div className="org-chart"><img src={scmCompanyLogo} alt="Logo SCM SARL" className="org-logo" /><h2>ORGANIGRAMME</h2><div className="org-line" /><div className="org-roots">{racines.map((bloc) => <OrgNodeEmploye key={bloc.id} bloc={bloc} blocs={blocs} />)}</div></div><p className="mt-4 text-center text-sm font-black text-muted-foreground">{organigramme?.description}</p></> : <p className="rounded-2xl bg-muted p-5 text-sm font-bold text-muted-foreground">Aucun organigramme publié pour le moment.</p>}</section>; }
