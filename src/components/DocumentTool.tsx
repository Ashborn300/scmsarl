  ...
  async function soumettre(event: React.FormEvent) {
    ...
    try {
      ...
      if (config.type === "rendu_3d") {
        ...
        const prompt = `Transform the uploaded 2D floor plan into a faithful isometric 3D architectural dollhouse render. Use the uploaded floor plan as the exact base. Preserve the exact position and proportions of all walls, rooms, doors, windows, stairs, balconies, corridors, bathrooms, kitchen areas, fixed furniture and built-in elements. Do not redesign, move, rotate, simplify, invent or remove any architectural element. Furnish each room realistically without changing the structure. Style: premium modern isometric cutaway apartment, warm lighting, white walls, wood flooring, elegant furniture, plants, realistic fixtures. Camera: 45-degree isometric top-down, open roof cutaway, full apartment visible, centered, dark studio background. Output: ultra detailed realistic 3D render, 16:9, sharp focus. No text, no labels, no watermark, no people, no layout distortion.${correction ? ` Correction request: ${correction}` : ""}`;
