...
const blocsOrganigrammeDefaut: BlocOrganigramme[] = [
  { id: "direction-generale", titre: "Directeur Général", niveau: 0, couleur: "bleu" },
  { id: "direction-technique", titre: "Directeur / Directrice Technique", niveau: 1, couleur: "vert", parentId: "direction-generale" },
  { id: "secretariat-general", titre: "Secrétaire G.", niveau: 2, couleur: "orange", parentId: "direction-technique" },
  { id: "secretariat-ad", titre: "Secrétaire A.D.", niveau: 2, couleur: "violet", parentId: "direction-technique" },
  { id: "chef-equipe", titre: "Chef d’équipe", niveau: 3, couleur: "turquoise", parentId: "secretariat-general" },
  { id: "disciplinaire", titre: "Disciplinaire", niveau: 4, couleur: "bleu", parentId: "chef-equipe" },
];

const niveauxOrganigramme = [0, 1, 2, 3, 4];

function normaliserBlocsOrganigramme(blocs: BlocOrganigramme[]) {
  return blocs.map((bloc) => {
    if (bloc.niveau === 0 || bloc.parentId) return bloc;
    const parent = blocs.find((item) => item.niveau === bloc.niveau - 1);
    return parent ? { ...bloc, parentId: parent.id } : bloc;
  });
}

function OrgNode({ bloc, blocs }: { bloc: BlocOrganigramme; blocs: BlocOrganigramme[] }) {
...
function OrganigrammeTool({ retour }: { retour: () => void }) {
  ...
  const blocsNormalises = normaliserBlocsOrganigramme(blocs);
  async function enregistrer(event: React.FormEvent) { event.preventDefault(); if (blocs.some((bloc) => !bloc.position)) return alert("Veuillez choisir la position de chaque bloc : En bas ou À côté."); const item = await enregistrerOrganigrammeEntreprise({ titre, description, blocs: blocsNormalises, actif: true }, editionId); setOrganigrammes((liste) => editionId ? liste.map((o) => o.id === item.id ? item : o) : [item, ...liste]); setEditionId(item.id); alert("Organigramme enregistré et publié sur les dashboards employés."); }
  ...
  function modifier(item: OrganigrammeEntreprise) { setEditionId(item.id); setTitre(item.titre); setDescription(item.description); setBlocs(item.blocs?.length ? normaliserBlocsOrganigramme(item.blocs) : blocsOrganigrammeDefaut); window.scrollTo({ top: 0, behavior: "smooth" }); }
  function changerNiveau(bloc: BlocOrganigramme, niveau: number) { if (blocs.filter((item) => item.id !== bloc.id && item.niveau === niveau).length >= 3) return alert("Maximum 3 blocs au même niveau."); const parent = blocs.find((item) => item.niveau === niveau - 1); setBlocs(blocs.map((b) => b.id === bloc.id ? { ...b, niveau, parentId: niveau === 0 ? undefined : parent?.id } : b)); }
  function ajouterBloc() { const niveau = niveauxOrganigramme.find((item) => item > 0 && blocs.filter((bloc) => bloc.niveau === item).length < 3) ?? niveauxOrganigramme.find((item) => blocs.filter((bloc) => bloc.niveau === item).length < 3); if (niveau === undefined) return alert("Chaque niveau contient déjà 3 blocs."); const parent = blocs.find((item) => item.niveau === niveau - 1); setBlocs([...blocs, { id: crypto.randomUUID(), titre: "Nouveau bloc", niveau, couleur: "bleu", parentId: niveau === 0 ? undefined : parent?.id }]); }
  return <main className="min-h-screen bg-background px-4 py-5 sm:px-6 lg:px-8 tool-organization-chart"><div className="mx-auto max-w-7xl"><button type="button" onClick={retour} className="mb-5 inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground transition hover:text-foreground"><ArrowLeft className="size-4" /> Retour au tableau de bord</button><div className="mb-6 rounded-3xl bg-tool-gradient p-6 text-tool-foreground shadow-tool lg:p-8"><span className="mb-4 inline-flex rounded-full bg-tool-foreground/15 px-3 py-1 text-xs font-bold uppercase tracking-wide">SCM SARL</span><h1 className="max-w-3xl text-3xl font-black lg:text-5xl">Organigramme de l’entreprise</h1><p className="mt-3 max-w-2xl text-sm opacity-90 lg:text-base">Composez les blocs et publiez le résultat chez tous les employés.</p></div><div className="grid gap-6 xl:grid-cols-[.9fr_1.1fr]"><form onSubmit={enregistrer} className="rounded-2xl border border-border bg-card p-5 shadow-document"><div className="grid gap-4"><label><span className="mb-1 block text-sm font-semibold">Titre</span><input className="form-control" value={titre} onChange={(e) => setTitre(e.target.value)} /></label><label><span className="mb-1 block text-sm font-semibold">Signature</span><input className="form-control" value={description} onChange={(e) => setDescription(e.target.value)} /></label></div><div className="mt-5 rounded-xl bg-muted p-3"><div className="mb-3 flex items-center justify-between"><h2 className="font-black">Blocs</h2><button type="button" className="mini-button" onClick={ajouterBloc}><Plus className="size-4" /> Ajouter</button></div><div className="space-y-3">{blocs.map((bloc) => { const parents = blocs.filter((item) => item.id !== bloc.id && item.niveau === bloc.niveau - 1); return <div key={bloc.id} className="grid gap-2 rounded-xl bg-card p-3 sm:grid-cols-[1fr_82px_1fr_112px_120px_40px]"><input className="form-control" value={bloc.titre} onChange={(e) => setBlocs(blocs.map((b) => b.id === bloc.id ? { ...b, titre: e.target.value } : b))} /><input className="form-control" type="number" min={0} max={4} value={bloc.niveau} onChange={(e) => changerNiveau(bloc, Number(e.target.value))} /><select className="form-control" value={bloc.parentId || ""} disabled={bloc.niveau === 0} onChange={(e) => setBlocs(blocs.map((b) => b.id === bloc.id ? { ...b, parentId: e.target.value || undefined } : b))}><option value="">Branche principale</option>{parents.map((parent) => <option key={parent.id} value={parent.id}>{parent.titre}</option>)}</select><select className="form-control" required value={bloc.position || ""} onChange={(e) => setBlocs(blocs.map((b) => b.id === bloc.id ? { ...b, position: e.target.value as BlocOrganigramme["position"] } : b))}><option value="">Position *</option><option value="bas">En bas</option><option value="cote">À côté</option></select><select className="form-control" value={bloc.couleur} onChange={(e) => setBlocs(blocs.map((b) => b.id === bloc.id ? { ...b, couleur: e.target.value as BlocOrganigramme["couleur"] } : b))}><option value="bleu">Bleu</option><option value="vert">Vert</option><option value="orange">Orange</option><option value="violet">Violet</option><option value="turquoise">Turquoise</option></select><button type="button" className="tool-action danger" onClick={() => setBlocs(blocs.filter((b) => b.id !== bloc.id))}><Trash2 className="size-4" /></button></div>; })}</div></div><button className="primary-action mt-5 w-full"><Save className="size-4" /> {editionId ? "Modifier" : "Publier"}</button><div className="mt-5 space-y-3">{organigrammes.map((item) => <article key={item.id} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-background p-3"><strong className="text-sm">{item.titre}</strong><div className="flex gap-2"><button type="button" className="tool-action" onClick={() => modifier(item)}><Pencil className="size-4" /></button><button type="button" className="tool-action danger" onClick={() => retirer(item.id)}><Trash2 className="size-4" /></button></div></article>)}</div></form><section className="rounded-2xl border border-border bg-card p-4 shadow-document"><OrganigrammePreview blocs={blocsNormalises} /><p className="mt-4 text-center text-sm font-black text-muted-foreground">{description}</p></section></div></div></main>;
}
