  ...
  async function soumettre(event: React.FormEvent) {
    ...
    try {
      ...
      if (config.type === "rendu_3d") {
        ...
        const prompt = `Transform the uploaded 2D floor plan into a highly detailed isometric 3D architectural dollhouse render.\n\nUse the uploaded floor plan as the exact architectural base.\n\nCRITICAL:\nThe 3D render must strictly preserve the original position of every element from the uploaded floor plan.\n\nKeep exactly the same placement of: walls, room divisions, doors, windows, stairs, balconies, corridors, bathrooms, kitchen, fixed furniture and built-in elements.\n\nDo not move, resize, rotate, simplify, invent, remove or reinterpret any architectural element. The 3D result must be a faithful isometric 3D conversion of the original plan, not a redesigned version.\n\nConvert each room into a realistic furnished interior while respecting the original layout. Add modern furniture only where it logically fits, without changing the structure.\n\nStyle: Premium 3D isometric cutaway apartment model, dollhouse view, realistic architectural visualization, cozy warm lighting, clean modern interior design, realistic wood flooring, white walls, elegant furniture, decorative plants, wall art, beds, sofa, dining table, kitchen appliances, bathroom fixtures and storage elements.\n\nCamera: 45-degree isometric top-down view, open roof cutaway, no ceiling, thick clean walls, entire apartment visible, centered composition, dark studio background.\n\nLighting: Warm interior lamps in each room, soft global illumination, cinematic but clean, realistic reflections, soft shadows, ambient occlusion, high detail.\n\nOutput: Ultra detailed, realistic 3D render, professional real estate marketing style, high resolution, sharp focus, 16:9 aspect ratio.\n\n--no text, labels, watermark, people, extra rooms, missing rooms, moved walls, moved doors, moved windows, distorted layout, wrong proportions, blurry render, low quality${correction ? `\n\nCorrection request: ${correction}` : ""}`;
        const planOptimise = await optimiserImagePourIA(imagesChamps.planImage, 1280, 0.9);
        const image = await genererImageOpenRouter({ data: { prompt, images: [planOptimise].filter(Boolean) } });
