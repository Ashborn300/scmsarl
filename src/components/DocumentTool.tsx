...
function OrgNode({ bloc, blocs }: { bloc: BlocOrganigramme; blocs: BlocOrganigramme[] }) {
  ...
  return <div className="org-branch"><div className={`org-node org-${bloc.couleur}`}><span className="org-icon"><UsersRound className="size-6" /></span><strong>{bloc.titre}</strong></div>{enfants.length > 0 && <div className="org-children">{enfants.map((enfant) => <OrgNode key={enfant.id} bloc={enfant} blocs={blocs} />)}</div>}</div>;
}
